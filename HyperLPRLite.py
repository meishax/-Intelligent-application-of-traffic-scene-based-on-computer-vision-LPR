...
class LPR():  # 定义LPR大类，整个大类在main.py中都会运行
    ...
    def cropImage(self, image, rect):  # 对车牌安全区域函数进行声明
        x, y, w, h = self.computeSafeRegion(image.shape, rect)
        return image[y:y + h, x:x + w]

    def detectPlateRough(self, image_gray, resize_h=720, en_scale=1.08,
                         top_bottom_padding_rate=0.05):  # 图像初步处理，以适应神经网络要求
        if top_bottom_padding_rate > 0.2:
            print("error:top_bottom_padding_rate > 0.2:", top_bottom_padding_rate)
            exit(1)  # 若需要裁剪图片大于0.2，则终止运行
        height = image_gray.shape[0]
        padding = int(height * top_bottom_padding_rate)  # 要裁剪掉图片的上下部占比
        scale = image_gray.shape[1] / float(image_gray.shape[0])
        image = cv2.resize(image_gray, (int(scale * resize_h), resize_h))
        image_color_cropped = image[padding:resize_h - padding, 0:image_gray.shape[1]]
        image_gray = cv2.cvtColor(image_color_cropped, cv2.COLOR_RGB2GRAY)  # rgb图像，Mat类型
        watches = self.watch_cascade.detectMultiScale(image_gray, en_scale, 2, minSize=(36, 9),
                                                      maxSize=(36 * 40, 9 * 40))
        cropped_images = []
        for (x, y, w, h) in watches:  # 重新设定的图像大小
            x -= w * 0.14
            w += w * 0.28
            y -= h * 0.15
            h += h * 0.3
    ...
    def fastdecode(self, y_pred):  # 定义置信度计算方法
        results = ""
        confidence = 0.0
    ...
    def model_seq_rec(self, model_path):  # 定义车牌初步定位函数，使用卷积神经网络和循环神经网络
    ...
    def model_finemapping(self):  # 定义车牌精细定位函数，卷积神经网络
        ...
        model = Model([input], [output])  # 车牌边界左右回归
        return model

    def finemappingVertical(self, image, rect):  # 图片进一步处理
        resized = cv2.resize(image, (66, 16))  # 将原来车牌图像resize大小：66*16*3
        resized = resized.astype(np.float) / 255  # 将原来灰度图颜色通道[0, 255]转化为float类型[0,1]
        ...
        rect[0] += res[0]
        image = image[:, H:T + 2]
        image = cv2.resize(image, (int(136), int(36)))  # 处理结果
        return image, rect

    def recognizeOne(self, src):  # 图像转置，维度进一步处理
        x_tempx = src
    ...
    def SimpleRecognizePlateByE2E(self, image):  # 输入为一个Mat类型的图片
        ...
        return res_set  # 输出为识别的车牌字符串，以及confidence可信度
